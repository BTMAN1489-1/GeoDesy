# GeoDesy corp.
## От автора
Проект __«Geodesy»__ представляет собой централизованной сервис по хранению и обработки информации о пунктах ГГС (геодезической сети). Основная идея заключается в исследовании и отслеживании состояния пунктов геодезической сети силами волонтеров: они на месте собирают информацию о пункте ГГС, после чего профессиональные геодезисты модерируют полученные данные. Результатом такого процесса является карточка пункта ГГС с краткой информацией. Каждый зарегистрированный доброволец видит на карте ближайшие пункты ГГС и при желании может заняться исследованием. Проект основан при поддержке университета, в частности, при поддержке местного геодезического сообщества.

__Важно:__ *В данный момент проект приостановлен, но альфа-версия backend составляющей была выложена в свободный доступ.*
